...
impl VoxelGrid {
        ...
        paths.par_chunks(64).for_each(|chunk| {
            for path in chunk {
                ...
                for i in 0..path.nodes.len() - 1 {
                    ...
                    let inv_seg_len_sq = if seg_len_sq > 0.0 {
                        1.0 / seg_len_sq
                    } else {
                        0.0
                    };
                    ...
                    for z in start_idx.z..=end_idx.z {
                        let voxel_z =
                            self.min_bound.z + z as f32 * self.voxel_size + self.voxel_size * 0.5;
        ...
        while !frontier.is_empty() {
            let next_frontier: Vec<usize> = frontier
                ...
                .map(|&idx| {
                    ...
                    if coord.x > 0 {
                        let n = idx - stride_x;
                        if self.try_set_state(n, VOXEL_EXTERNAL) {
                            local.push(n);
                        }
                    }
                    if coord.x < dim.x - 1 {
    ...
    fn dilate_visible_region(&self, radius_voxels: i32) {
        ...
        let frontier: Vec<usize> = (0..self.voxels.len() * VOXELS_PER_WORD)
            .into_par_iter()
            .filter_map(|idx| {
...
pub fn optimize_paths(mut paths: Vec<ExtrusionPath>, config: &Config) -> Vec<ExtrusionPath> {
    ...
    paths.par_sort_unstable_by(|a, b| {
        let p1 = if a.nodes.is_empty() {
            Vec3::ZERO
        } else {
            a.nodes[0].pos
        };
        let p2 = if b.nodes.is_empty() {
            Vec3::ZERO
        } else {
            b.nodes[0].pos
        };
        ...
        .filter(|path| {
            ...
            for node in &path.nodes {
                if let Some(idx) = grid
                    .world_to_grid(node.pos)
                    .map(|uv| grid.get_index_from_coord(uv))
                {
        ...
        for x in 0..size {
            for y in 0..size {
                paths.push(ExtrusionPath {
                    nodes: vec![
                        PathNode {
                            pos: Vec3::new(x as f32 * gap, y as f32 * gap, 0.0),
                            width: 0.4,
                            height: 0.2,
                        },
                        PathNode {
                            pos: Vec3::new(x as f32 * gap + gap, y as f32 * gap, 0.0),
                            width: 0.4,
                            height: 0.2,
                        },
                    ],
                ...
                paths.push(ExtrusionPath {
                    nodes: vec![
                        PathNode {
                            pos: Vec3::new(x as f32 * gap, y as f32 * gap, size as f32 * gap),
                            width: 0.4,
                            height: 0.2,
                        },
                        PathNode {
                            pos: Vec3::new(x as f32 * gap + gap, y as f32 * gap, size as f32 * gap),
                            width: 0.4,
                            height: 0.2,
                        },
                    ],

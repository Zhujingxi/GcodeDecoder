...
pub fn generate_mesh(paths: &[ExtrusionPath], config: &Config) -> Vec<Triangle> {
    ...
    let process_path = |path: &ExtrusionPath| -> Vec<Triangle> {
        ...
        for i in 0..node_count {
            ...
            // Define Frame
            let up = Vec3::Z;
            let safe_up = if dir.abs_diff_eq(Vec3::Z, config.up_vector_epsilon)
                || dir.abs_diff_eq(Vec3::NEG_Z, config.up_vector_epsilon)
            {
                Vec3::X
            } else {
                up
    ...
    {
        paths.iter().flat_map(process_path).collect()
    }
}
